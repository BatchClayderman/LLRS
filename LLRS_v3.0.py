...
try:
	from numpy import array, concatenate, diag, dot, full, hstack, ones, split as np_split, sum as np_sum, tile, where, zeros # dot and np_sum require " % q"
...
try:
	...
	print("Please install the library named \"scipy\" properly before this script can be run. ")
	print("Exception(s): ")
	print(e)
	print("Please press enter key to exit. ")
	try:
...
# Child Functions #
def solveEqu(A, B, q) -> array:
	m, n = A.shape
	constraint = LinearConstraint(A = hstack((A, diag(full(shape = m, fill_value = -q)), )), lb = B, ub = B)
	result = milp(c = zeros(n + m), integrality = ones(n + m), bounds = Bounds(lb = zeros(n + m)), constraints = constraint)
	x, f = np_split(result.x.astype(int), (n, ))
...
def TrapGen(pars:PARS) -> tuple:
	...
	e = solveEqu(A_0i, zeros((n, ), dtype = "int"), q) # Ae = 0 (mod q) | size = (m / 2, 1)
...
def ExtBasis(B_0i:array, pars:PARS) -> array:
	q = pars.getQ()
	n = pars.getLowerN()
	T__B0i = solveEqu(B_0i, zeros((n, ), dtype = "int"), q) # size = (m, 1)
	return T__B0i
...
def RandBasis(B_0i:array, pars:PARS) -> array:
	...
	n = pars.getLowerN()
	m = pars.getM()
	b = solveEqu(B_0i, zeros((n, ), dtype = "int"), q) # Bb = 0 (mod q) | size = (m, 1)
