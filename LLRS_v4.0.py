...
try:
	from numpy import arange, array, concatenate, diag, dot, eye, fill_diagonal, full, hstack, kron, ones, split as np_split, squeeze, sum as np_sum, tile, where, zeros # dot, kron, and np_sum require " % q"
...
def ExtBasis(F_B0:array, T_B0:array, B_0:array, q:int) -> array:
	W = zeros((B_0.shape[1], F_B0.shape[1]))
	for j in range(F_B0.shape[1]):
		W[:, j] = solveEqu(B_0, squeeze(F_B0[:, j]), q)
